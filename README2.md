...
### 3、HashMap的put过程***
    ...
    4、如果链表长度超过阈值，则进行扩容操作。
    5、完成put操作：完成put操作，HashMap的容量和负载因子可能会发生变化。
### 4、hashmap调用get方法一定安全吗
    不一定。如果多个线程同时调用get方法，可能会导致数据不一致或死循环。
    调用get方法有两点需要注意：
    1、空指针异常：如果键为null，则调用get方法会抛出空指针异常。
    2、线程安全：如果多个线程同时调用get方法，可能会导致数据不一致或死循环。
### 5、HashMap一般用什么做key,为什么String适合做Key*
    用String做key是一个好的选择，因为String是线程安全的，并且String的hashCode()方法已经经过优化，所以String的hashCode()方法生成的哈希值是唯一的，并且比较快。
### 6、HashMap为什么要用红黑树而不是平衡二叉树*
    HashMap使用红黑树是为了提高查询效率，因为红黑树在插入、删除和查找时，时间复杂度都是O(logn)，
    而平衡二叉树在插入和删除时，时间复杂度都是O(logn)，但查找时，时间复杂度是O(n)。
### 7、分段锁怎么加锁*
    分段锁是Java中一种用于并发编程的锁机制，它通过将锁分割成多个 smaller lock 来提高并发性能。
    在Java中，分段锁是通过java.util.concurrent.locks.ReentrantReadWriteLock来实现的。
## Set
### Set集合有什么特点？如何实现key无重复的
    Set集合的特点是元素不重复，即同一个元素不能重复出现。
    可以使用Set集合来保证key不重复，Set集合不允许有重复的元素，因此，当向Set集合中添加一个元素时，如果该元素已经存在，则添加操作会失败，不会添加任何元素。
    会先根据元素的hashCode()方法计算出元素的哈希值，然后根据哈希值计算出元素的索引，再通过equals方法来判断元素是否重复。